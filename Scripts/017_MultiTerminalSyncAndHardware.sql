/*
    ...
    Run: sqlcmd -S .\SQLEXPRESS -E -d PointOfSale -i Scripts\017_MultiTerminalSyncAndHardware.sql
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
GO
...
USING (SELECT N'Schema.Phase30Applied' AS ConfigKey) AS source
...
ON target.ConfigKey = source.ConfigKey
WHEN MATCHED THEN
    UPDATE SET ConfigJson = N'25', UpdatedAt = GETUTCDATE()
WHEN NOT MATCHED THEN
    INSERT (ConfigKey, ConfigJson, UpdatedAt)
    VALUES (N'Schema.Version', N'25', GETUTCDATE());
GO

PRINT N'Phase 30 multi-terminal sync and hardware tables applied.';
GO

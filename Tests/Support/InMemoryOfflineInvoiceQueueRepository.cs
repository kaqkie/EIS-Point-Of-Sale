...
/// <summary>
/// In-memory FIFO queue repository mirroring SQL eligibility rules for integration tests.
/// </summary>
public sealed class InMemoryOfflineInvoiceQueueRepository : IOfflineInvoiceQueueRepository
{
    ...
    public IReadOnlyList<OfflineInvoiceQueueItem> Snapshot()
    {
        lock (_gate)
        {
    ...
    private static OfflineInvoiceQueueItem Clone(OfflineInvoiceQueueItem source) =>
        new()
        {
            Id = source.Id,
            ...
            FiscalResponseJson = source.FiscalResponseJson
        };
}
